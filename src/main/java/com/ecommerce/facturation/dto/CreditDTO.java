...
public class CreditDTO extends TransactionDTO {
    ...
    @Valid
    List<CreditBillingToPayDTO> creditBillingToPays;

    public CreditDTO(@NotNull(message = "Full name cannot be null") @DecimalMin(value = "0.0", inclusive = false, message = "Amount must be greater than 0") BigDecimal amount, @Valid BankAccountDTO sender, @Valid BankAccountDTO receiver, PaymentStatus paymentStatus, TransactionalType transactionalType, @Valid InvoiceDTO invoice, List<CreditBillingToPayDTO> debitBillingToReceiveDTOS) {
